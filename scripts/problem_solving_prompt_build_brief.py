#!/usr/bin/env python3
"""Compile PROMPT inputs into a minimal patch while preserving the compiler baseline."""
...
BASELINE_PROMPT_MARKER = "[보존할 Compiler baseline 프롬프트]"
...
def _baseline_prompt(baseline: Mapping[str, Any]) -> str:
    value = baseline.get("final_prompt")
...
최종 프롬프트를 작성하지 않는다. Prompt Compiler baseline은 이미 작동하는 기본 자산으로
보존하고, 사용자 요청·Goal Ledger·선택적 주 경로 결과에서 baseline에 추가하거나 바로잡아야
할 차이만 짧은 수정 패치로 정리한다.
...
[보존 대상 Prompt Compiler baseline]
...
def deterministic_prompt_build_brief(
    ...
    current_step = str(ledger.get("current_step") or "").strip()
    return {
        "version": 1,
        "goal": str(ledger.get("current_goal_hypothesis") or "").strip(),
        "core_procedure": [current_step] if current_step else [],
        "supporting_inputs": [],
...
def render_prompt_build_brief(brief: Mapping[str, Any]) -> str:
    lines = ["# Prompt Build Patch", "", f"## 목표\n{brief['goal']}", ""]
    labels = (
        ("core_procedure", "추가·수정할 핵심 절차"),
        ("supporting_inputs", "추가할 보조 입력·도구"),
        ("fixed_constraints", "반드시 보존할 고정 조건"),
        ("output_contract", "완료·출력 계약"),
        ("defaults_and_exceptions", "추가할 기본값과 예외"),
        ("exclusions", "제외 범위"),
        ("upstream_context", "새로 반영할 주 경로 내용"),
    )
...
def build_prompt_executor_from_brief(
    ...
    if contract_suffix:
        contract_note = (
            "\n\n[검증 계약 사용법]\n"
            "Result Contract는 완료 여부를 확인하는 계약이다. baseline의 절차를 없애거나 같은 수의 "
            "제목으로 다시 펼치지 말고, 필요한 누락만 패치하면서 충족한다."
            + contract_suffix
        )

    if baseline_prompt:
        task_surface = f"""{BASELINE_PROMPT_MARKER}
{baseline_prompt}

{BRIEF_MARKER}
{json.dumps(dict(brief), ensure_ascii=False, indent=2)}"""
        mode_instruction = """아래 baseline 프롬프트를 기본 자산으로 보존하고 Prompt Build Brief를 수정 패치로만 사용한다.
최종 프롬프트를 처음부터 다시 쓰거나 더 짧게 만드는 작업이 아니다."""
        preservation_rules = """1. baseline의 도메인 절차, 판단 순서, 예외, 안전장치, 출력 구조를 기본적으로 그대로 보존한다.
2. Brief에 있는 내용 중 baseline에 실제로 빠졌거나 충돌하는 항목만 최소 범위로 추가·수정한다.
3. 간결함, 문체 통일, 제목 수 감소만을 이유로 baseline 내용을 삭제·일반화·재배열하지 않는다.
4. 수정 초안을 baseline과 내부 비교해 목표 충족, 절차 완전성, 고정 조건, 실행 가능성 중 하나라도 불필요하게 나빠지면 해당 수정을 되돌린다.
5. 명확한 실질 개선이 없으면 baseline을 한 글자도 바꾸지 않고 그대로 최종 프롬프트로 반환한다.
6. 이 단계의 결과는 적용 품질 검증 전 후보이므로, 검증되지 않은 개선을 성공한 것으로 설명하지 않는다."""
    else:
        task_surface = f"""{BRIEF_MARKER}
{json.dumps(dict(brief), ensure_ascii=False, indent=2)}"""
        mode_instruction = """사용 가능한 baseline 프롬프트가 없으므로 아래 Prompt Build Brief에서
복사해 바로 쓸 최종 프롬프트 하나를 새로 만든다."""
        preservation_rules = """1. Brief의 목표, 고정 조건, 완료 조건을 빠짐없이 반영한다.
2. 핵심 작업 절차를 중심에 두고 보조 규칙과 출력 형식을 종속시킨다.
3. 없는 사실·수치·도구 사용을 만들지 않는다."""
...
[baseline 보존·수정 원칙]
{preservation_rules}
7. limitations에는 최종 프롬프트 자체에 실제로 남은 한계만 쓴다.
8. 내부 시스템·브리지·receipt·생성 과정 설명을 사용자용 프롬프트에 넣지 않는다.
...
class PromptBuildBriefEngine:
    """Intercept PROMPT executor calls and preserve the baseline plus a focused patch."""
    ...
    def __init__(
        ...
        *,
        ...
        self._baseline_prompts: dict[str, str] = {}
    ...
    def execute(self, prompt: str, run_dir: Path, invocation: Any) -> dict[str, Any]:
        ...
        baseline_prompt = self._baseline_prompts.get(stage, "")
        ...
        record.setdefault("deliveries", []).append(
            {
                ...
                "baseline_preserved_in_executor": bool(baseline_prompt),
            }
        )
    ...
    def _compile(
        ...
        baseline_prompt = _baseline_prompt(baseline)
        ...
        generation = "model_compiled_patch"
        ...
        try:
            ...
            generation = "deterministic_patch_fallback"
        ...
        if baseline_prompt:
            baseline_prompt_path = root / "baseline_prompt.md"
            baseline_prompt_path.write_text(baseline_prompt.rstrip() + "\n", encoding="utf-8")
        ...
        source_record = {
            ...
            "baseline_prompt_sha256": _sha256_text(baseline_prompt) if baseline_prompt else None,
        ...
        self._baseline_prompts[stage] = baseline_prompt
        self._records[stage] = {
            "stage": stage,
            "generation": generation,
            "generation_error": generation_error,
            "preservation_mode": (
                "baseline_plus_patch" if baseline_prompt else "patch_only_no_baseline"
            ),
            "promotion_status": "pending_applied_evaluation",
            ...
            "baseline_prompt_path": (
                relative(baseline_prompt_path) if baseline_prompt_path else None
            ),
    ...
    def record(self) -> dict[str, Any] | None:
        ...
        return {
            "version": 2,
            "status": "applied",
            "input_contract": "baseline_plus_prompt_patch",
            "promotion_policy": (
                "생성 결과는 적용 전 후보이며, 동일 입력 A/B에서 baseline보다 명확히 나을 때만 승격한다."
            ),
