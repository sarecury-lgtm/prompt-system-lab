(() => {
  ...
  const STORAGE_KEY = "psos-chatgpt-manual-v4";
  ...
  function originalRequest(form = elements.form) {
  ...
  function classify(request, form = elements.form) {
    ...
    const decisionAction = /(살까|매수|진입|매도|팔까|대기|회피|보유|손절|구매할까|사도 될까)/i.test(text);
    const oneTargetSignal = /(오늘|지금|내일|실적|차트|이 종목|이 제품|이거|토스트|tost)/i.test(text);
...
- 설명만 하지 말고 사용자가 바로 쓸 수 있는 답을 제시한다.`;
  ...
  function visibleCurrentResult() {
    ...
    return [result, candidateState].filter(Boolean).join("\n\n").slice(0, 18000);
  }
...
교정이 반영된 완성된 새 답변만 제시한다. 변경 내역 설명이나 메타 설명은 붙이지 않는다.`;
  ...
  toggle.className = "workflow-manual-toggle manual-v4-toggle";
  toggle.innerHTML = `
    <input id="chatgpt-manual-enabled" type="checkbox">
    <span>
      <strong>Codex 없이 계속</strong>
      <small>이 박스 안에서 질문부터 입력하고 ChatGPT로 넘깁니다.</small>
    </span>
  `;
  ...
  panel.className = "chatgpt-manual-panel manual-v4-panel";
  panel.hidden = true;
  panel.innerHTML = `
    <div class="chatgpt-manual-heading">
      <div>
        <span class="workflow-kicker">Codex 없는 간편 실행</span>
        <h3>여기에 질문을 쓰고 ChatGPT로 보내세요.</h3>
        <p id="chatgpt-manual-summary">아래 첫 번째 칸이 실제 입력칸입니다.</p>
      </div>
      <span id="chatgpt-manual-route" class="workflow-badge"></span>
    </div>

    <label class="field-label manual-v4-request-label" for="chatgpt-manual-request">
      <span>무엇을 해결할까요?</span>
      <textarea id="chatgpt-manual-request" rows="5" maxlength="10000" placeholder="예: 토스트 주식을 오늘 매수하는 게 좋을까? 내일 실적 발표야."></textarea>
    </label>

    <ol class="manual-v4-progress" aria-label="수동 실행 단계">
      <li data-step="1"><strong>1</strong><span>질문 보내기</span></li>
      <li data-step="2"><strong>2</strong><span>답변 붙이기</span></li>
      <li data-step="3"><strong>3</strong><span>끝내기</span></li>
    </ol>

    <section class="manual-v4-step" data-manual-step="1">
      <div class="manual-v4-step-head">
        <div><strong>1. ChatGPT에서 실행</strong><p>위 질문을 작업 종류에 맞는 지시문으로 바꿔 복사합니다.</p></div>
        <button id="chatgpt-manual-open" type="button" disabled>복사하고 ChatGPT 열기</button>
      </div>
      <p id="chatgpt-manual-attachment-note" class="manual-v4-attachment-note" hidden></p>
      <details class="manual-v4-packet-details">
        <summary>보낼 내용 확인·직접 복사</summary>
        <textarea id="chatgpt-manual-packet" rows="12" readonly></textarea>
        <button id="chatgpt-manual-copy" type="button" class="secondary-button" disabled>내용만 복사</button>
      </details>
    </section>

    <section class="manual-v4-step" data-manual-step="2">
      ...
      <div class="manual-v4-finish-actions">
    ...
    <section id="chatgpt-manual-followup" class="manual-v4-step manual-v4-followup" data-manual-step="3" hidden>
      <label class="field-label" for="chatgpt-manual-correction">
        <span>바꿀 점 한 줄</span>
        <textarea id="chatgpt-manual-correction" rows="3" placeholder="예: 결론을 먼저 말하고 차트 위험을 더 크게 반영해."></textarea>
      </label>
  ...
  const manualRequest = panel.querySelector("#chatgpt-manual-request");
  ...
  const openButton = panel.querySelector("#chatgpt-manual-open");
  const copyButton = panel.querySelector("#chatgpt-manual-copy");
  const progressItems = panel.querySelectorAll(".manual-v4-progress li");
  ...
  function restore() {
    try {
      ...
      manualRequest.value = typeof saved.request === "string" ? saved.request : "";
      packet.value = typeof saved.packet === "string" ? saved.packet : "";
      response.value = typeof saved.response === "string" ? saved.response : "";
      correction.value = typeof saved.correction === "string" ? saved.correction : "";
    } catch (_error) {
  ...
  function updateProgress() {
    ...
    else if (packet.value.trim()) active = 2;
  ...
  function rebuildPacket({ includeCurrentResult = state.includeCurrentResult, form = elements.form } = {}) {
    const parsed = stripAttachmentBlock(manualRequest.value);
    ...
    packet.value = state.request
      ? buildInitialPacket(state.route, state.request, previous, state.attachmentNames)
      : "";
    openButton.disabled = !packet.value.trim();
    copyButton.disabled = !packet.value.trim();
    updateHeader();
    updateProgress();
    persist();
  }

  function syncFromOriginal(form = elements.form, includeCurrentResult = false) {
    const source = originalRequest(form);
    if (source) manualRequest.value = source;
    rebuildPacket({ includeCurrentResult, form });
  }
  ...
  function setEnabled(enabled, message = "") {
    ...
    if (state.enabled) {
      if (!manualRequest.value.trim()) syncFromOriginal();
      else rebuildPacket();
      if (message) status.textContent = message;
      window.requestAnimationFrame(() => manualRequest.focus());
    }
    persist();
  }
  ...
  async function copyAndOpen() {
    ...
    if (!packet.value.trim()) {
      status.textContent = "위의 ‘무엇을 해결할까요?’ 칸에 질문을 입력해 주세요.";
      manualRequest.focus();
      return;
    }
    ...
    try {
      ...
      panel.querySelector(".manual-v4-packet-details").open = true;
      packet.focus();
      packet.select();
  ...
  document.addEventListener(
    ...
    (event) => {
      ...
      syncFromOriginal(event.target, false);
      panel.hidden = false;
  ...
  manualRequest.addEventListener("input", () => {
    if (elements.request.value !== manualRequest.value) elements.request.value = manualRequest.value;
    state.includeCurrentResult = false;
    rebuildPacket();
  });
  ...
  correction.addEventListener("input", persist);
  ...
  continueButton.className = "secondary-button manual-v4-continue";
  ...
  continueButton.addEventListener("click", () => {
    setEnabled(true);
    syncFromOriginal(elements.form, true);
    panel.scrollIntoView({ behavior: "smooth", block: "start" });
  });
  ...
  fallbackButton.addEventListener("click", () => {
    ...
    syncFromOriginal(elements.form, false);
    panel.scrollIntoView({ behavior: "smooth", block: "start" });
  });
