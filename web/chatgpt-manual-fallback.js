(() => {
  ...
  function currentRequest(form = elements.form) {
    if (form === promptUi.form) return promptUi.request.value.trim();
    return elements.request.value.trim();
  }
  ...
  function routeInstructions(route) {
    if (route === "research") {
      return `최신 정보가 결과를 바꿀 수 있는 조사 요청이다.
- 웹 검색을 실제로 수행한다.
- 날짜, 가격, 판매 여부, 규정처럼 변할 수 있는 사실은 출처와 확인 시점을 함께 적는다.
- 확인한 사실과 추론을 구분한다.
- 자료가 충돌하면 숨기지 말고 어떤 근거를 더 신뢰했는지 설명한다.
- 최종적으로 사용자가 바로 판단할 수 있는 결론을 낸다.`;
    }
    if (route === "candidate") {
      return `추천·구매·비교 요청이다. 한 번에 최종 결론으로 뛰어가지 말고 먼저 후보 작업대를 만든다.
- 실제로 유효한 후보 3~7개만 남긴다.
- 각 후보에 candidate-001 형식의 고정 ID를 부여한다.
- 이름, 출처 링크, 확인된 속성, 장점, 위험, 아직 확인할 점을 적는다.
- 후보를 많이 나열하지 말고 왜 남겼는지 설명한다.
- 첫 답변에서는 후보 작업대까지만 제시하고 사용자의 짧은 교정을 기다린다.
- 다음 대화에서 후보 ID를 유지하며 제외, 조건 변경, 부분 재조사, 검증을 필요한 범위에만 적용한다.`;
    }
    if (route === "prompt") {
      return `다른 AI가 바로 실행할 최종 프롬프트를 만드는 요청이다.
- 사용자의 상위 목적, 고정 조건, 실제 수행 절차, 완료 조건을 먼저 내부적으로 정리한다.
- 같은 의미를 반복하지 않는다.
- 프롬프트 작성 과정이나 내부 구조 설명은 출력하지 않는다.
- 최종 출력은 복사해 바로 사용할 수 있는 프롬프트 본문 하나만 제공한다.`;
    }
    if (route === "write") {
      return `로컬 파일을 직접 열 수 없는 일반 ChatGPT 환경에서 수행하는 파일 작업이다.
- 사용자가 첨부하거나 붙여 넣은 기존 파일을 기준으로 작업한다.
- 기존 기능을 빠뜨리지 말고 요청한 부분만 수정한다.
- 필요한 파일이 없으면 정확히 어떤 파일이 필요한지만 짧게 말한다.
- 가능하면 전체 교체 파일 또는 적용 가능한 통합 diff를 제공한다.
- 실제 로컬 적용이 끝났다고 주장하지 않는다.`;
    }
    return `현재 대화와 제공된 자료만으로 요청을 직접 해결한다.
- 가장 가능성 높은 의도를 먼저 잡고 진행한다.
- 결론을 바꾸는 정보가 부족한 경우에만 질문한다.
- 설명만 하지 말고 사용자가 바로 쓸 수 있는 결과를 완성한다.
- 확실한 사실, 사용자 제공 정보, 추론을 섞지 않는다.`;
  }
...
아래 이전 결과를 처음부터 새로 만들지 말고, 사용자의 교정을 반영해 필요한 부분만 수정한다.
${route === "candidate" ? "후보 ID와 이미 확인된 정보는 유지하고, 제외된 후보를 다시 살리지 않는다." : "이미 맞는 내용과 구조는 보존한다."}
...
[출력]
교정이 반영된 새 결과만 제시한다. 무엇을 했다는 메타 설명은 최소화한다.`;
  }
  ...
  const saveButton = panel.querySelector("#chatgpt-manual-save");
  const followupButton = panel.querySelector("#chatgpt-manual-followup-button");

  let activeRoute = "direct";
  let activeRequest = "";

  async function copyPacket(open = false) {
    ...
    try {
      ...
      status.textContent = "자동 복사에 실패했습니다. 지시문을 직접 선택해 복사해 주세요.";
    }
  }
  ...
  function setEnabled(enabled, reason = "") {
    ...
    document.body.classList.toggle("chatgpt-manual-enabled", Boolean(enabled));
  ...
  function saveManualResult() {
    ...
    showCompleted({
      ...
      artifacts: [],
      evidence: [],
      limitations: ["일반 ChatGPT에서 수동으로 실행하고 결과를 다시 붙여 넣은 기록입니다."],
  ...
  document.addEventListener(
    ...
    (event) => {
      ...
      panel.hidden = false;
      status.textContent = "지시문을 일반 ChatGPT에 보내고 답변을 아래에 붙여 넣으세요.";
  ...
  promptUi.request.addEventListener("input", () => {
    if (toggleInput.checked && selectedEngineMode() === "integrated") refreshPacket(promptUi.form);
  });
  copyButton.addEventListener("click", () => copyPacket(false));
  openButton.addEventListener("click", () => copyPacket(true));
  saveButton.addEventListener("click", saveManualResult);
  followupButton.addEventListener("click", makeFollowup);
  ...
  function updateErrorFallback() {
    ...
    fallbackButton.hidden = elements.error.hidden || !unavailable;
  }
