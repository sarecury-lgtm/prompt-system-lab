...
3. core_procedure는 서로 다른 판단 단계를 합치지 말고 권장 12개 이하로 정리하되 필요한 단계는 누락하지 않는다.
4. supporting_inputs에는 절차 수행에 필요한 자료, 입력 형태, 분석 요소, 도구만 두며 권장 12개 이하로 정리하되 필요한 항목은 누락하지 않는다.
...
7. 나머지 output_contract에는 사용자가 실제로 비교·판단·행동하는 데 필요한 산출물만 두며 권장 12개 이하로 정리하되 필요한 항목은 누락하지 않는다.
8. defaults_and_exceptions에는 누락 정보 처리처럼 결과가 달라지는 기본값만 두며 권장 12개 이하로 정리하되 필요한 항목은 누락하지 않는다.
9. exclusions에는 목표 밖의 작업만 두며 권장 12개 이하로 정리하되 필요한 항목은 누락하지 않는다.
10. upstream_context에는 최종 프롬프트에서 새로 사용할 검증된 내용만 두며 권장 12개 이하로 정리하되 필요한 항목은 누락하지 않는다.
