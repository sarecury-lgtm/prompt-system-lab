...
기록에는 이전·새 bundle SHA-256, 후보 ID, 원본 페이지, 수집 시각, 추가된 evidence ID와 중복 URL이 남는다.
...
- 수집한 외부 이미지 URL의 파일 자체는 아직 run 폴더에 복제하지 않는다. 주소가 만료되거나 hotlink가 차단되면 원본 페이지에서 다시 확인해야 한다.
